...
    @State private var cachedSelectedDateTasks: [Task] = [] // Cache to prevent recomputation
    @State private var lastCachedDate: Date? = nil
...
        
        // Use cache if date hasn't changed
        if let lastDate = lastCachedDate, calendar.isDate(lastDate, inSameDayAs: selectedDate) {
            return cachedSelectedDateTasks
        }
        
...
        }
        
        let sortedTasks = filteredTasks.sorted { $0.startTime < $1.startTime }
        
        // Update cache
        cachedSelectedDateTasks = sortedTasks
...
                Spacer()
                
                // Calendar Button (DEC 2025) and Today button on right
                HStack(spacing: 8) {
                    Button(action: {
